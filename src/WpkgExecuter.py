...
class WpkgExecuter():
    ...
    def Query(self, handle=None):
        ...
        for line in lines:
            ...
            if line != '':
                ...
                if line.startswith(excludes):
                    ...
                    # Package Name
                    value = line
                # Decode from Windows CodePage and encode into utf-8
                # Now we only have to detect current windows codepage?
                cleaned_lines.append(value.decode('cp850').encode('utf-8'))
        ...
        # Create XML FILE WITH DATA, NEEDED?!?
        root = ET.Element('packages')
        comment = ET.Comment('Generated by python....')
        root.append(comment)
        for entry in composite_list:
            package = ET.Element('package')
            package.set('id', entry[0].decode('utf-8'))
            package.set('version', entry[1])
            package.set('action', entry[2])
            root.append(package)
        tree = ET.ElementTree(root)
        tree.write(open(r'c:\test.xml', 'w'), encoding="UTF-8", xml_declaration=True)
